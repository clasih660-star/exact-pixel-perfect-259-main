    ...
    <DashboardShell config={dashboardConfigs.platform_admin} title="KingPin Course">
      ...
      {!course ? (
        <Card className="border-[#DDE7F0] bg-white/90">
          <CardContent className="p-6 text-sm text-[#475569]">Course not found.</CardContent>
        </Card>
      ) : (
        <>
          ...
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <SummaryCard
              label="Lessons"
              value={course.modules.reduce((sum, module) => sum + module.lessons.length, 0)}
              icon={<BookOpen className="h-5 w-5" />}
            />
            <SummaryCard
              label="AI tools covered"
              value={course.toolUniverse.length}
          ...
          <div className="mt-8">
            ...
            <KingpinCertificatePreview
              course={course}
              learnerName="KingPin Graduate"
              certificateNumber="KP-2026-AI-2048"

...
export const Route = createFileRoute("/institutions/register")({
  head: () => ({
    meta: [
      ...
      { name: "keywords", content: "register school, register institution, AI classroom setup, virtual classroom for schools, Klassruum registration" },
      { name: "author", content: "Klassruum" },
      { name: "robots", content: "index, follow" },
      { property: "og:type", content: "website" },
      { property: "og:title", content: "Register Your Institution — Klassruum" },
      { property: "og:description", content: "Create AI-powered virtual classrooms for your institution." },
...
const BENEFITS = [
  { icon: "🎓", title: "AI teacher-led lessons", body: "Virtual teacher delivers structured lessons with voice, captions, and real-time interaction." },
  { icon: "📝", title: "Materials to lessons", body: "Upload PDFs, slides, and documents — AI generates ready-to-teach structured lessons." },
  { icon: "♿", title: "Built-in accessibility", body: "Live captions, transcripts, learning modes for deaf, blind, ADHD, and more." },
  { icon: "📊", title: "Progress tracking", body: "Track lessons started, questions asked, checkpoints reached, and areas needing review." },
  { icon: "🔒", title: "Data control", body: "Institution-owned data with role-based access, encrypted storage, and audit logging." },
  { icon: "🌍", title: "Works everywhere", body: "Browser-based, low-bandwidth friendly, runs on modest devices and patchy connections." },
];
    ...
    <div className="flex min-h-screen">
      ...
      <div className="hidden lg:flex lg:w-[520px] xl:w-[580px] flex-col justify-between overflow-hidden bg-[#1A3233] p-10 relative">
        ...
        <div className="relative z-10">
          ...
          <p className="mt-4 text-base leading-relaxed text-[#A3ADAD]">
            Register your school, university, tutoring center, or training provider. Start delivering structured, accessible, teacher-led lessons at scale.
          </p>
          ...
          <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2">
            {BENEFITS.map((b) => (
              <div key={b.title} className="rounded-xl border border-white/10 bg-white/5 p-4">
                ...
                <p className="mt-1 text-xs leading-relaxed text-[#A3ADAD]">{b.body}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="flex flex-1 items-start justify-center bg-white px-6 py-12 overflow-y-auto">
        <div className="w-full max-w-[560px] space-y-8">
          ...
          <div>
            <h2 className="text-2xl font-bold tracking-tight text-[#1A3233]">Register your institution</h2>
          ...
          >
            ...
            <section className="rounded-2xl border border-[#d1eceb] bg-white p-6 shadow-sm">
              ...
              <div className="mt-6 grid gap-5 sm:grid-cols-2">
                <div className="space-y-1.5 sm:col-span-2">
                  <label htmlFor="iname" className="block text-sm font-medium text-[#1A3233]">Institution name *</label>
                  <input
                    id="iname"
                    required
                    maxLength={200}
                ...
                <div className="space-y-1.5">
                  <label className="block text-sm font-medium text-[#1A3233]">Institution type *</label>
                  <select
                    value={form.type}
                ...
                <div className="space-y-1.5">
                  <label htmlFor="ilearners" className="block text-sm font-medium text-[#1A3233]">Number of learners</label>
                  <input
                    id="ilearners"
                    type="number"
                    min={0}
                ...
                <div className="space-y-1.5">
                  <label htmlFor="icountry" className="block text-sm font-medium text-[#1A3233]">Country *</label>
                  <input
                    id="icountry"
                    required
                    value={form.country}
                ...
                <div className="space-y-1.5">
                  <label htmlFor="icity" className="block text-sm font-medium text-[#1A3233]">City *</label>
                  <input
                    id="icity"
                    required
                    value={form.city}
                ...
                <div className="space-y-1.5 sm:col-span-2">
                  <label className="block text-sm font-medium text-[#1A3233]">Preferred use case</label>
                  <select
                    value={form.preferred_use_case}
                    onChange={(e) => setForm({ ...form, preferred_use_case: e.target.value as typeof form.preferred_use_case })}
                    className="w-full rounded-xl border border-[#d1eceb] bg-white px-4 py-3 text-sm text-[#1A3233] outline-none transition-all focus:border-[#1F7C80] focus:ring-2 focus:ring-[#1F7C80]/10"
                  >
                    {USE_CASES.map(([v, l]) => (
                      <option key={v} value={v}>{l}</option>
            ...
            <section className="rounded-2xl border border-[#d1eceb] bg-white p-6 shadow-sm">
              ...
              <div className="mt-6 grid gap-5 sm:grid-cols-2">
                <div className="space-y-1.5 sm:col-span-2">
                  <label htmlFor="aname" className="block text-sm font-medium text-[#1A3233]">Full name *</label>
                  <input
                    id="aname"
                    required
                ...
                <div className="space-y-1.5">
                  <label htmlFor="aemail" className="block text-sm font-medium text-[#1A3233]">Email *</label>
                  <input
                    id="aemail"
                    type="email"
                    required
                ...
                <div className="space-y-1.5">
                  <label htmlFor="aphone" className="block text-sm font-medium text-[#1A3233]">Phone *</label>
                  <input
                    id="aphone"
                    required
                    value={form.phone}
                ...
                <div className="space-y-1.5 sm:col-span-2">
                  <label htmlFor="apass" className="block text-sm font-medium text-[#1A3233]">Password *</label>
                  <input
                    id="apass"
                    type="password"
                    required

...
export const reviewAdmissionApplication = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }: any) => {
    ...
    if (appError || !application)
      throw new Error(appError?.message || "Admission application not found.");
    const access = await requireInstitutionAdminAccess(
      supabaseAdmin,
...
export const convertAdmissionToEnrollment = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }: any) => {
    ...
    if (!["accepted", "waitlisted", "reviewing"].includes(application.status)) {
      throw new Error(
        "Only accepted, waitlisted, or reviewing applications can continue to enrollment.",
      );
    }
    ...
    if (!profile?.id) {
      ...
      const { data: invite, error: inviteError } = await supabaseAdmin
        ...
        .insert({
          ...
          message:
            "Your admission application has been approved. Join Klassruum to complete enrollment.",
          metadata_json: {
      ...
      if (inviteError || !invite)
        throw new Error(inviteError?.message || "Could not create invite.");
      inviteId = invite.id;
    ...
    const { data: updated, error: updateError } = await supabaseAdmin
      ...
      .update({
        ...
        metadata_json: {
          ...((application.metadata_json as Record<string, unknown>) ?? {}),

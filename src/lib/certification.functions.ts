...
export const upsertCompletionRule = createServerFn({ method: "POST" })
    ...
    .handler(async ({ data, context }: any) => {
        ...
        const { data: rule, error } = data.rule_id
            ? await supabaseAdmin.from("completion_rules").update(payload).eq("id", data.rule_id).select("*").single()
...
export const issueCertificate = createServerFn({ method: "POST" })
    ...
    .handler(async ({ data, context }: any) => {
        ...
        const [existingCertRes, enrollmentRes, progressRes, quizRes, ruleRes, studentProfileRes] = await Promise.all([
            supabaseAdmin
                .from("certificates")
                .select("*")
                .eq("institution_id", data.institution_id)
            ...
            data.completion_rule_id
                ? supabaseAdmin.from("completion_rules").select("*").eq("id", data.completion_rule_id).single()
                : Promise.resolve({ data: null, error: null }),
            supabaseAdmin.from("profiles").select("email, full_name").eq("id", data.student_id).maybeSingle(),
        ]);
        ...
        const maxProgress = Math.max(0, ...((progressRes.data ?? []).map((r: any) => r.progress_percentage ?? 0)));

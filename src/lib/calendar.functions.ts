...
export const scheduleInstitutionSession = createServerFn({ method: "POST" })
    ...
    .handler(async ({ data, context }: any) => {
        const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
        const access = await requireInstitutionStaffAccess(supabaseAdmin, data.institution_id, context.userId);
        ...
        const { data: session, error: sessionError } = await supabaseAdmin
            ...
            .select("*")
            .single();
        if (sessionError || !session) throw new Error(sessionError?.message || "Could not schedule session.");
        ...
        const { data: event, error: eventError } = await supabaseAdmin
            ...
            .select("*")
            .single();
        if (eventError || !event) throw new Error(eventError?.message || "Could not create calendar event.");
        ...
        for (const enrollment of enrollments ?? []) {
            ...
            for (const offset of data.reminder_offsets_minutes) {
                const scheduledFor = new Date(new Date(data.starts_at).getTime() - offset * 60 * 1000).toISOString();
        ...
        if (reminderRows.length > 0) {
            const { error: reminderError } = await supabaseAdmin.from("session_reminders").insert(reminderRows);
